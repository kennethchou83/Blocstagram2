...
//  Blocstagram
//
//  Created by Kenneth Chou on 7/18/16.
...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
       ...
       self.tableView.decelerationRate = 5.0;
...
-(void) loadingVisibleImages{
{
    ...
    if ((self.tableView.dragging == NO & indexPath == nil)|| (self.tableView.dragging == YES))
        
    
        for (NSIndexPath *index in indexPath) {
        
               Media *mediaItem = [DataSource sharedInstance].mediaItems[index.row];
...
- (void) observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (object == [DataSource sharedInstance] && [keyPath isEqualToString:@"mediaItems"]) {
        ...
        // We know mediaItems changed.  Let's see what kind of change it is. NSKeyValueChange can be int, and unsignedIntegerValue can be intValue, but once using int, unsighnedInteverValue has to be changed to intValue.
